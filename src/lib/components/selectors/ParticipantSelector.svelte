...
<Section name="Select Team">
	...
	<div class="split">
		<button
			on:click={() => (participant.alliance = 'Red')}
			class:red={participant.alliance == 'Red'}>Red Alliance</button
		>
		<button
			on:click={() => (participant.alliance = 'Blue')}
			class:blue={participant.alliance == 'Blue'}>Blue Alliance</button
		>
	</div>
	<button
		on:click={() => (manuallyOverriding = !manuallyOverriding)}
		class:active={manuallyOverriding}>Manual Override</button
	>
</Section>
